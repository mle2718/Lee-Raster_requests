---
title: "Tables and Reports for Framework 7: Selected Months"
author: Min-Yang Lee
...
output_folder_name = file.path(project_root_path,"raw_data/Herring/Framework7/SONonly")
...
```{r subset_months, eval=T}
FIN<-FIN[which(FIN$MONTH %in% c("9","10","11")),]
FIN_backup <- FIN
...
```{r subset_revenuefile, include=FALSE, eval = TRUE}
REVENUEFILE <- REVENUEFILE[(which(REVENUEFILE$MONTH %in% c("9","10","11"))),]
...
if (length(proj_area_name) == 1) {
  save(REVENUEFILE, file= file.path(output_revenue, paste0("REVENUEFILESON_", proj_area_name, ".rdata"))) # save revenuefile if number of areas is 1
} else if(length(proj_area_name) > 1 & overlap_data_source == "folder") {
  save(REVENUEFILE, file= file.path(output_revenue, paste0("REVENUEFILESON_", basename(overlap_data_path), ".rdata"))) # save revenuefile if more than 1 area and overlap data comes from a folder - save as name of folder
} else {
  save(REVENUEFILE, file= file.path(output_revenue, paste0("REVENUEFILESON_", proj_area_name[1],"_and_other_areas", ".rdata"))) # save revenuefile if more than 1 area and any other edge cases
}
...
NOTE: These data used to construct these figures have been filtered to the months under consideration (Sept, Oct, Nov).
