...
if ((base_year %in% 1947:2022 == FALSE)) {stop("Base year not between 2022 and 1947 or not given..")}
...
if (overlap_data_source == "folder") {
  if (!dir.exists(overlap_data_path)) {
  stop("Not connected to shared drive or overlap_data_path") # checks that the overlap path exists to see if the user has entered the password for the shared drive
  }
}
...
```{r create_connection_if_querying, eval = eval_create_connection,include=FALSE}  
