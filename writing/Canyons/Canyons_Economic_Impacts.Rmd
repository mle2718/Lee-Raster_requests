...
A reasonable baseline is the *status quo* condition.  82% of the NCSMNM is contained within the Georges Bank Deep-Sea Coral Protection Area and fishing with all gear except red crab pot/trap gear continues to be prohibited in this area in the baseline.  Landings and revenue from recent years are typically used to construct the baseline economic conditions.  However, the Georges Bank Deep-Sea Coral Area has been closed since July 26, 2021, so data from recent years (when fishing *was* permitted the Deep-Sea Coral area) may not give the full picture of the baseline conditions.  Therefore, in addition to summarizing the average landings and revenue (real 2020 USD), by FMP in Table \ref{tab:baseline_revenues}, the average revenue (real 2020 USD) derived from  the Georges Bank Deep-Sea Coral Protection Area is also presented in column four of Table \ref{tab:baseline_revenues}.  It is likely that a large proportion of the fishing activity in column four has been migrated to a different part of the ocean.  
```{r baseline_revenues}
Baseline_rev <- CFDBS_FMP %>%
  ...
  mutate(Revenue = as.character(round(Revenue,2)) ) %>%
...
kbl(Baseline_rev, booktabs=T,  align=c("l",rep('r',times=4)), digits=3, caption = "Baseline Regional Landings (millions of pounds) and Value (Millions of Real 2020 USD) are derived from average 2017-2020 dealer reports.  Revenue derived from the GB Deep Sea Coral Protection Area during this time period is also presented to illustrate the magnitude of fishing that occured in this area that is currently closed to commercial fishing.")  %>%
      kable_styling(full_width = T) %>% 
      row_spec(0,bold=TRUE) 
```


```{r GB_DSC, out.width = "40%", fig.show='hold',fig.align="center", fig.cap="\\label{fig:GB_Deep_Sea_Coral} Revenue in real 2020USD (left) and percentage terms (right) , by FMP, derived from the Georges Bank Deep-Sea Coral Protection Area.", eval=FALSE}
...
Therefore, we therefore characterize the fishing effort, in terms of landings, that would occur inside this area. We use commercial fishing data based on vessel trip reports from  `r START.YEAR` - `r END.YEAR` time due to the changing status of fishing after  `r END.YEAR`.  These projected landings and revenues were constructed using the methods described in [@DePiper2014; @Benjamin2018] and were also used in a he similar analysis in support of the NEFMC's Habitat Omnibus Amendment 2 [-@NEFMC_habitat_2016_V6]. In brief, fishing vessels report the location (coordinate) where the majority of their fishing effort was located.  Because fishing activity does not occur at a single point, a model, based on data collected through the observer program is used to account for this by allocating the fishing activity over a more realistic area. The amount of allocated effort that occurs inside the NCSMNM addition over the `r START.YEAR` to `r END.YEAR` period is used to construct projected landings and revenue that would occur if the NCSMNM addition was reopened to commercial fishing.

These data *under-represent* fishing activity attributable to the lobster fishery and pot/trap gears because vessel operators that hold a lobster permit and no other federal permit are not required to fill out vessel trip reports.  Of the 113 fishing vessels with an Area 3 lobster permit in 2018, 90 were required to file VTRs because they held other Northeast Region permits.

Table \ref{tab:top_fmp_revenue_table} projects landings and revenue that is likely to occur inside the NCSMNM addition if reopened.   These tables provide insight into likely annual flows of benefits of resuming fishing in the NCSMNM addition and the types of firms to which those benefits would accrue *in the baseline*. Figures \ref{fig:top_fmp_landings_plots} and \ref{fig:top_fmp_revenue_figure}  describe landings and  landings from `r START.YEAR` - `r END.YEAR` that occured within the NCSMNM boundary addition disaggregated by FMP and year.  These provide insight into the likely year-to-year variation of those benefits.


Some types of gear (say midwater trawl or handline) has minimal effects on the ocean bottom compared to others (say Trawl or Dredge). Table \ref{revenue_table_by_gear} describes the economic activity, disaggregated by gear, projected to occur inside the NCSMNM addition if reopened. These proved insight into both the effects of fishing effort on the sea bottom and the types of commercial users that may benefit from the opening of the NCSMNM.

To convert the annual flow of uncertain future economic activity in Tables \ref{fig:top_fmp_landings_plots} and \ref{fig:top_fmp_revenue_figure} into  current values, it is necessary to adjust for both the probability that fishing will occur and when it will occur.  We refrain from an assessment of either of these figures, but note that both factors will reduce the values shown in in Tables \ref{fig:top_fmp_landings_plots} and \ref{tab:revenue_table_by_gear}.
...
# set dynamic varible names (cant use paste0 in rename in tidyverse)
names(allyrrev_table)[names(allyrrev_table) == "All Year Revenue"] <- paste0(numyear, " Average Projected Revenue")

allyrrev_table <-allyrrev_table %>%
  left_join(allyrland_table, by="FMP") %>%
  relocate(`All Year Revenue`, .after=last(col))

kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=2)), caption = "Projected commercial landings and revenue  (Real USD) that would occur in the NCSMNM addition if it was reopened to fishing based on 2012-2016.")  %>%
      kable_styling(full_width = T) %>% 
      row_spec(0,bold=TRUE)  %>%
      row_spec(nrow(allyrland_table),bold=T)
...
# set dynamic varible names (cant use paste0 in rename in tidyverse)
names(allyrland_table)[names(allyrland_table) == "All Year Total"] <- "Average Projected Landings Landings"
...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=2)), caption = "Projected commercial landings (pounds) and revenue that would occur in the NCSMNM addition if it was reopened to fishing based on 2012-2016.")  %>%
      kable_styling(full_width = T) %>%
...
The proposed action would make not changes to the types of fishing activities that can be currently conducted in the NCSMNM; fishing is likely to continue at levels described in Table \ref{tab:baseline_revenues}.  However, the proposed action removes the risk that future administrations will open NCSMNM to commercial fishing by presidential proclamation. 

##### Benefits of regulation
Relative to the baseline, the benefits of the proposed action include annual bequest values, existence values, and incremental contribution of the NCSMNM habitat to the productivity of commercial valuable resources that are harvested outside of the NCSMNM areas.  These are adjusted by the probability that the protections for NCSMNM would be lifted in the future.  All of these values are difficult to quantify.
...
```{r percent_of_revenue_by_firm, eval = T, fig.cap="\\label{fig:percent_of_revenue_by_firm} Annual Firm Revenue Percentage Boxplots, only firms with some activity inside NCSMNM Addition"}  
...
```{r rev_by_firm, eval = T, fig.cap="\\label{fig:revenue_by_firm} Annual Firm Revenue Boxplots, only firms with some activity inside NCSMNM Addition"}  
