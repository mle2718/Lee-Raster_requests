...
Commerical fishing, except for red crab and lobster, in the Northeast Canyons and Seamounts National Monument (NCSMNM) was prohibited from September 15, 2016 to June 5, 2020 (Presidential Proclamation 9496).  From June 6, 2020 to October 8, 2021, commercial fishing was allowed (Proclamation 10049).   The entirety of the NCSMNM is currently closed to fishing by Presidential Proclamation 10287 under the Antiquities Act.  82% of the NCSMNM is within the Georges Bank Deep Sea Coral Area through the NEFMC's Habitat Omnibus Amendment 2 and commercial fishing except red crab has been closed since July26, 2021.  18% of the NCSMNM is not contained in the Georges Bank Deep Sea Coral Area is the focus of the economic analysis and is referred to as the "NCSMNM Addition" (Figure \ref{fig:map_NCSMNM}). 
...
An Initial Regulatory Flexibility Analysis (IRFA; section NNNN) has been prepared to further evaluate the economic impacts of the various alternatives presented in this document on regulated small business entities. 
...
Fishing in NCSMNM (with limited temporary exceptions) is currently prohibited by presidential proclamation and the propsed action does not change the currently allowed fishing areas.  Instead, it codifies into regulation Proclamation 9496, as amended by Proclamation 10287, which directs the Secretaries of Commerce and Interior to promulgate regulations necessary for the proper care and management of the Monument.  The costs, relative to the *status quo*, are opportunity cost of fishing adjusted by the likelihood that future administrations choose to reopen the NCSMNM to commercial fishing. Fishing in much of the NCSMNM (with limited exceptions) is currently also prohibited through the Georges Bank Deep-Sea Coral Protection Area, which was implemented through the NEFMC's Omnibus Deep-Sea Coral Amendment (86 FR 33553).

The primary benefits likely derive from an increased level of certainty in improving the condition of corals, sponges, and other deep-sea fauna in NCSMNM and indirectly improving the production of other species which rely on this habitat. Due to data limitations we were not able to quantify the costs or benefits of this rule associated with habitat conservation. Still, we believe that the overall impact of the proposed rule falls well short of the annual \$100 million threshold for a significant economic action because Vessel Trip Reports indicate that fishing vessels would derive \$1-2 million in annual gross revenue from the NCSMNM Addition. 
...
See Section 3.0 for a description of the fisheries.

#### Statement of the Problem (Market Failure)
...
A Net Present Value (NPV) framework provides a useful theoretical framework for understanding the economic effects of potential changes in human activity allowed inside NCSMNM; however, a full NPV analysis is not performed. 

The seafloor in NCSMNM can be thought of as a capital asset.  Beyond it's value derived from the unique ecological resources that are subject to scientific interest, the seafloor ecoystem services, including habitat for species that are commercially valuable.  The quality of the capital asset decreases after fishing occurs when bottom-contact from fishing gear damages the unique oceanographic formations or destroys marine life.  Because fauna in the area (like deep-sea coral and sponges) grow, the habitat can slowly regenerate after these events. However, coral and sponges are fragile, slow-growing, and relatively immobile, so even small amounts of intermittent activity may have long-lasting, negative impacts on the stock of habitat in NCSMNM.  
...
Therefore, the economic analysis focuses on the 18% of NCSMNM that is not contained with the Georges Bank Deep-Sea Coral Protection Area.  The analysis also focuses on the long-run and examines the continued closure of the NCSMNM to all gears, including Red Crab and Lobster.

#### Other Alternatives 

#### Baseline Description

A reasonable baseline is the *status quo* condition.  82% of the NCSMNM is contained within the Georges Bank Deep-Sea Coral Protection Area and fishing with all gear except red crab pot/trap gear continues to be prohibited in this area in the baseline.  Table \ref{tab:baseline_revenues} summarizes the average landings and revenue (real 2020 USD), by FMP, constructed from the NMFS dealer databases from 2017 to 2020.  We note that this table *includes* fishing activity that occurred inside the Georges Bank Deep-Sea Coral Protection Area. That fishing activity is described in Figure \ref{fig:GB_Deep_Sea}.  
...
Baseline_rev <- CFDBS_FMP %>%
  ...
  summarise(Landings=mean(LANDINGS),
            Value=mean(VALUE),
            GB=mean(inOMNIBUSRev)) %>%
  arrange(desc(Value)) %>%
  mutate(Value = as.character(round(Value,2)) ) %>%
  mutate(GB = as.character(round(GB,2)) ) %>%
  mutate(Value = ifelse(FMP == "Red Crab", '(C)',Value))%>%
  mutate(Value = ifelse(FMP == "Tilefish", '(C)',Value)) %>%
  mutate(GB = ifelse(FMP == "Tilefish", '(C)',GB)) %>%
  mutate(GB = ifelse(FMP == "Red Crab", '(C)',GB))




# Here is where we want to join things to the new REVENUEFILE

kbl(Baseline_rev, booktabs=T,  align=c("l",rep('r',times=3)), digits=2, caption = "2017-2020 Average Annual Landings (millions of pounds) and Value (Millions of Real 2020 USD) and Revenue derived from the GB Deep Sea Coral Protection Area")  %>%
...
Therefore,we therefore characterize the fishing effort, in terms of landings, that would occur inside this area. We use commercial fishing data based on vessel trip reports from  `r START.YEAR` - `r END.YEAR` time due to the changing status of fishing after  `r END.YEAR`.   These data *under-represent* fishing activity attributable to the lobster fishery and pot/trap gears because vessel operators that hold a lobster permit and no other federal permit are not required to fill out vessel trip reports.  Of the 113 fishing vessels with an Area 3 lobster permit in 2018, 90 were required to file VTRs because they held other Northeast Region permits.   
...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Average Value (Real 2020 USD)")  %>%
      kable_styling(full_width = F) %>% 
      row_spec(0,bold=TRUE)  %>%
      row_spec(nrow(allyrland_table),bold=T)
...
kbl(Total_REV, booktabs=T,  align=c("l",rep('r',times=2)), caption = "2012-2016 Year Average Landings (Millions of Pounds) and Revenue (Millions of 2021 Dollars), All Areas")  %>%
...
Figures \ref{fig:landings_gear_plot} to \ref{fig:revenue_gear_plot} describe landings from `r START.YEAR` to `r END.YEAR` that occurred within the NCSMNM boundary addition. They are disaggregated by gear to provide some insight into the possible declines in the stock of habitat in the area if fishing resumed.  We note that American Lobster and Red Crab fisheries would not be impacted unil September 15, 2023.  Annual lobster landings hav averaged `r lobster_lbs` pounds per year and \$ `r lobster_rev ` per year over the `r START.YEAR` to `r END.YEAR` time period.  Red crab landings cannot be reported due to data confidentiality reasons and are aggregated into the "Other NER" and have averaged less than `r other_lbs` pounds per year and \$`r other_rev` per year over this time period.

Some types of gear (say midwater trawl or handline) has minimal effects on the ocean bottom compared to others (say Trawl or Dredge). Tables \ref{tab:landings_table_by_gear} and \ref{tab:revenue_table_by_gear} present totals over this five year time period.
...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Total Revenue (2021 USD) , Select Gear Types")  %>%
      kable_styling(full_width = F) %>%
      row_spec(nrow(allyrrev_table)-1,hline_after=TRUE) %>% 
...
The proposed action would make not changes to the types of fishing activities that can be currently conducted in the NCSMNM: commercial fishing, with some exceptions, is currently prohibited.  There are no costs of the proposed regulation on current fishing activity.  Similarly, there are no benefits of regulation on the with respect to the ecosystem services currently derived from the protected areas.  
...
2.  Opening NCSMNM to commercial fishing changes the flow of benefits from the NCSMNM.  Opening NCSMNM would reduce the annual compliance (opportunity) costs for the commercial fishery to zero. Consumer surplus would be derived from increases in landings. Opening NCSMNM to fishing would also reduce the ecosystem benefits due to the reduced protection of the unique habitats in NCSMNM. The changes in benefits associated with reduced protection and the changes in compliance costs are also difficult to describe quantitatively. 
...
Relative to the baseline, the benefits of the proposed action is the discounted flow of bequest values, existence values, and incremental contribution of the NCSMNM habitat to the productivity of commercial valuable resources that are harvested outside of the NCSMNM areas, adjusted by the probability that the protections for NCSMNM would be lifted in the future.  All of these values are difficult to quantify.
...
We are unable to estimate the changes societal welfare from fishing within in the areas of the preferred alternative. Instead we present an estimate of revenue and landings that might return to the NCSMNM addition as an estimate of the potential scale of the industry.  These revenue and landings figures are contained in Figures \ref{fig:top_fmp_landings_plots} -\ref{fig:revenue_gear_plot} and Tables \ref{tab:top_fmp_landings_table} to \ref{tab:revenue_table_by_gear}. 
...
We also plot the percentage of each firm's total commercial fishing revenue derived from inside the NCSMNM addition area (Figure \ref{fig:percent_of_revenue_by_firm}).  Only firms with revenue from the area are included in the plots; these plots are companions to Table \ref{tab:AnyInside}.  The area inside the boxes of indicate capture the 25$^th$-75$^th$ percentiles, indicating that,  after excluding firms that do not use the NCSMNM addition area, most firms derive a small proportion of their total revenue from the NCSMNM addition area. 

```{r percent_of_revenue_by_firm, results='asis', eval = T, fig.cap="\\label{fig:percent_of_revenue_by_firm} Annual Firm Revenue Percentage Boxplots,, only firms with some activity inside NCSMNM Addition"}  
...
Similarly, box-whisker plots of firm's total commercial fishing revenue derived from inside the NCSMNM addition area (Figure \ref{fig:revenue_by_firm}) illustrate that most of the firms earn less than \$30,000 in gross receipts from the NCSMNM addition.

```{r revenue_by_firm, eval = T, fig.cap="\\label{fig:revenue_by_firm} Annual Firm Revenue Boxplots, only firms with some activity inside NCSMNM Addition"}  
...
```{r Rev_and_Pct_scatter,eval = T, fig.cap="\\label{fig:REV_PCT_scatter} Scatterplot of Percentage of Firm Revenue Inside NCSMNM against Annual Firm Revenue Inside NCSMNM, only firms with some activity inside NCSMNM Addition"}
...
PCT_Scatter<-PermitInsideRev %>%
  ...
  mutate(
    ...
    AllREV=AllREV/num_years
  )

ggplot(PCT_Scatter, aes(x=InsideREV, y=Percent)) +
  geom_point() + 
  labs(x = "Average Annual ('000s) Revenue Inside NCSMNM",
...
```{r count_years, fig.cap="\\label{fig:nyears} Histogram of the number of years that firms were active in the NCSMNM addition. "}
...
ggplot(count_years, aes(x=n)) + 
  ...
  labs(x = "Number of Years (2012-2016) fished in NCSMNM Addition" ,
       y = "Number of Firms Active in NCSMNM Addition")  
