...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=2)), caption = "Projected annual commercial landings and revenue  (Real USD) that would occur in the NCSMNM addition if it was reopened to fishing based on 2012-2016 patterns of fishing.")  %>%
      kable_styling(full_width = T) %>% 
      row_spec(0,bold=TRUE)  %>%
      row_spec(nrow(allyrland_table),bold=T)
...
See Section 3.0 for a description of the fisheries.  Section [Baseline-Description] summarizes relevant aspects of the fishery.
...
Similarly, box-whisker plots of firm's total commercial fishing revenue derived from inside the NCSMNM addition area (Figure \ref{fig:revenues_by_firm}) illustrate that most of the firms earn less than \$30,000 in gross receipts from the NCSMNM addition.

```{r rev_by_firm, eval = T, fig.cap="\\label{fig:revenues_by_firm} Annual Firm Revenue Boxplots, only Small firms with some activity inside NCSMNM Addition"}  
...
1.  Few firms use the NCSMNM area at all (Tables \ref{tab:Firm_average_in_Area} and \ref{tab:AnyInside})
...
3.  As a percentage of gross reciepts, small firms derive slightly more revenue that large firms from NCSMNM  (Table \ref{tab:AnyInside}).
4.  A handful of small firms use the NCSMNM addition for moderate amount of their fishing \ref{fig:percent_of_revenue_by_firm}.
5.  However, the firms that do derive more than 20\% of their gross receipts from the NCSMNM addition are quite small in size.
