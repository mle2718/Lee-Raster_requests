...
```{r top_fmp_landings_plots, eval = top_fmp_section} 
##########################################################################
...
```{r top_fmp_revenue_figure, eval = top_fmp_section} 
...
Figures \ref{fig:top_fmp_landings_plots} and \ref{fig:top_fmp_revenue_figure} describe landings and revenue from 2012 to 2016 that occured within the NCSMNM boundary addition disaggregated by FMP and year. These figures provide insight into annual flows of benefits of resuming fishing in the NCSMNM addition and the types of firms to which those benefits would accrue *in the baseline*.  Tables \ref{tab:top_fmp_landings_table} and \ref{tab:top_fmp_revenue_table} present totals over this five year time period.
...
kbl(allyrland_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016-Year Total Landings (Pounds)")  %>%
...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Total Value (Real USD)")  %>%
      kable_styling(full_width = F)
...
Figures \ref{fig:landings_plot_by_gear} to \ref{fig:revenue_plot_by_gear} describe landings from 2012 to 2020 that occurred within the NCSMNM boundary addition. They are disaggregated by gear to provide some insight into the possible declines in the stock of habitat in the area if fishing resumed.  Some types of gear (say midwater trawl or handline) has minimal effects on the ocean bottom compared to others (say Trawl or Dredge). Tables \ref{tab:landings_table_by_gear} and \ref{tab:revenue_table_by_gear} present totals over this five year time period.

```{r landings_plot_by_gear, eval = select_gear_section}  
...
```{r revenue_plot_by_gear, eval = select_gear_section}  
###################################################################
...
kbl(allyrland_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Total Landings")  %>%
      kable_styling(full_width = F)
```
...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Total Revenue, Select Gear Types")  %>%
      kable_styling(full_width = F)
cat("\n\n\n\n\n\n")
...
The directly regulated entities are the firms that currently hold at least 1 Northeast US fishing permit that could be used to fish in the Northeast Canyons and Seamounts. A few permit categories (Lobster: A1, A2, A4, A5, A5W, A6, and AOC; General Category Scallop B) were not included when constructing the set of directly regulated entities because these permit categories to not enable a vessel to fish in the Northeast Canyons and Seamounts areas.  There are a total of 1,338 small firms and 11 Large firms.  1,024 of the small firms derive the majority of their revenue from commercial fishing operations.  314 derive the majority of their revenue from for-hire recreational activities.  Firms that are classified as "for-hire" may still fish commerically -- these firms are included in the tabulation of directly regulated entities, although their main source of income (for-hire fishing) would not be directly impacted  by the proposed action. 
...
Tables \ref{tab:Firm_average_in_Area}  and \ref{tab:AnyInside} suggest that the additional NCSMNM area are not often used by fishing firms.  Compliance costs, measured as foregone revenue from this area, are expected to be low.  On average, firms derived less than 1\% of annual fishing revenue from this area, and typically substantially less than this.  
