...
Neither of these works prop properly though

```{r child=here("writing","Canyons", "data_process_Canyons.Rmd")}
```


```{r, child='data_process_Canyons.Rmd'}
```
...
CFDBS_FMP<-readRDS(CFDBS_FMP, file= file.path(output_revenue, paste0("CFDBS_FMP.Rds")) ) 
...
```{r create_map_of_lease_area, eval = T,fig.cap="\\label{fig:map_NCSMNM} Northeast Canyons and Seamounts National Monument Addition.  This area is part of the NCSMNM that does not lie inside the Georgest Bank Deep Sea Coral Area."} 
...
```{r top_fmp_revenue_figure, eval = top_fmp_section, fig.cap="\\label{fig:top_fmp_revenue_figure} Revenue (Real 2021USD)  by FMP, Northeast Canyons and Seamounts Addition Area."} 
...
kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Average Value (Real 2021 USD)")  %>%
      kable_styling(full_width = F) %>% 
      row_spec(0,bold=TRUE)  %>%
      row_spec(nrow(allyrland_table),bold=T)
...
We also plot the percentage of each firm's total commercial fishing revenue derived from inside the NCSMNM addition area (Figure \ref{fig:percent_of_revenue_by_firm}).  Only firms with revenue from the area are included in the plots; these plots are companions to Table \ref{tab:AnyInside}.  The box-whisker plots indicate, even after excluding firms that do not use the NCSMNM addtion area,  firms derive a small proportion of their total revenue from the NCSMNM addition area. 
...
```{r revenue_by_firm, results='asis', eval = T, fig.cap="\\label{fig:revenue_by_firm} Annual Firm Revenue Boxplots, only firms with some activity inside NCSMNM Addition"}  
