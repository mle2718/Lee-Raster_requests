...
GARFO is leading an omnibus amendment to the Fishery Management Plans of the MA and NE fishery Management Councils.  The proposed action would change affect vessels that hold a permit for any of the thirteen fishery management plans or for the American Lobster and Jonah Crab fisheries.  The proposed action would close all commercial fishing activities; Red crab and American lobster commercial fishing would be exempt until September 15, 2023. Recreational fishing activities are not affected.  
...
A Net Present Value framework provides a useful theoretical construct for understanding the economic effects of changes in human activity allowed inside NCSMNM. 

The seafloor in NCSMNM can be thought of as capital asset.  The seafloor provides ecosystem services, such as habitat, for species that are commercially valuable.  The quality of the capital asset decreases after fishing occurs when bottom-contact from gear damages habitat oceanographic formation or destroys marine life.  Because fauna in the area (like deep-sea coral and sponges) grow, the habitat can slowly regenerate after these events. However, coral and sponges are fragile, slow-growing, and relatively immobile, so even small amounts of intermittent activity may have long-lasting, negative impacts on the stock of habitat in NCSMNM.  

The economic activity in the NCSMNM can be characterized as series of flows. Each year, benefits accrue to the nation.  These benefits may take the form of producer and consumer surplus derived from extractive uses, like commercial fishing.  Benefits may also accrue though non-extractive uses through bequest and existence values described in the previous section.

Understanding the NPV of economic activity in NCSMNM arising from any policy is then an exercise of discounting and summing the flows of benefits (accruing from extractive and non-extractive uses) over time. An economically optimal policy with respect to commercial fishing in the NCSMNM balances the additional benefits of fishing (consumer and producer surpluses) against the costs of fishing (the actual costs, plus damages to the habitat).  
...
```{r top_fmp_revenue_figure, eval = top_fmp_section, fig.cap="\\label{fig:top_fmp_revenue_figure}Revenue by FMP, Northeast Canyons and Seamounts addition Area"} 
...
Figures \ref{fig:top_fmp_landings_plots} and \ref{fig:top_fmp_revenue_figure}  describe landings and  landings from `r START.YEAR` - `r END.YEAR` that occured within the NCSMNM boundary addition disaggregated by FMP and year. Tables \ref{tab:top_fmp_landings_table} and \ref{tab:top_fmp_revenue_table} present averages over this five year time period.  These figures *under-represent* fishing activity attributable to the lobster fishery and pot/trap gears because vessel operators that hold a lobster permit and no other federal permit are not required to fill out vessel trip reports. 

These figures provide insight into annual flows of benefits of resuming fishing in the NCSMNM addition and the types of firms to which those benefits would accrue *in the baseline*.  
...
names(allyrland_table)[names(allyrland_table) == "All Year Total"] <- paste0(numyear, " Year Average Landings")

kbl(allyrland_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Year Average Landings (Pounds)")  %>%
...
# set dynamic varible names (cant use paste0 in rename in tidyverse)
names(allyrrev_table)[names(allyrrev_table) == "All Year Revenue"] <- paste0(numyear, " Year Average Revenue")

kbl(allyrrev_table, booktabs=T,  align=c("l",rep('r',times=1)), caption = "2012-2016 Average Value (Real USD)")  %>%
      kable_styling(full_width = F)
...
Table \ref{tab:total_fishing_START} describes landings and real revenues over the  `r START.YEAR` - `r END.YEAR` time period, disaggregated by Fishery Management Plan, in all areas. These data *under-represent* fishing activity attributable to the lobster fishery because vessel operators that hold a lobster permit and no other federal permit are not required to fill out vessel trip reports.


```{r total_fishing_START}
...
kbl(Total_REV, booktabs=T,  align=c("l",rep('r',times=2)), caption = "2012-2016-Year Average Landings (Millions of Pounds) and Revenue (Millions of Dollars), All Areas")  %>%
      kable_styling(full_width = F) %>%
...
Figures \ref{fig:landings_plot_by_gear} to \ref{fig:revenue_plot_by_gear} describe landings from `r START.YEAR` to `r END.YEAR` that occurred within the NCSMNM boundary addition. They are disaggregated by gear to provide some insight into the possible declines in the stock of habitat in the area if fishing resumed.  Some types of gear (say midwater trawl or handline) has minimal effects on the ocean bottom compared to others (say Trawl or Dredge). Tables \ref{tab:landings_table_by_gear} and \ref{tab:revenue_table_by_gear} present totals over this five year time period.

```{r landings_plot_by_gear, eval = select_gear_section,  fig.cap="\\label{fig:landings_plot_by_gear} Landings by Gear, Northeast Canyons and Seamounts addition Area"}  
...
```{r revenue_plot_by_gear, eval = select_gear_section, fig.cap="\\label{fig:revenue_plot_by_gear}Revenue by Gear, Northeast Canyons and Seamounts addition Area"}  
###################################################################
...
A reasonable baseline is *status quo* condition in which fishing in those areas continues to be prohibited by Presidential Proclamation.  As seen from historical proclamations, future administrations can easily and quickly change the set of allowed activities in the area to include commercial fishing.
...
However, the proposed action removes the risk that future administrations will open NCSMNM to commercial fishing by presidential proclamation. The expected economic effects here are best characterized in two components:
...
We are unable to estimate the changes societal welfare from fishing within in the areas of the preferred alternative. Instead we present an estimate of potential displaced revenue and landings as an estimate of the potential scale of the industry.  These revenue and landings figures are contained in Figures \ref{fig:top_fmp_landings_plots} -\ref{revenue_plot_by_gear} and Tables \ref{top_fmp_landings_table} to \ref{revenue_table_by_gear} 
...
DRE_2012_2016Firms <- Firm_activity %>%
  ...
  summarise(Firms=n_distinct(affiliate_id),
    ...
    mutate(across(c(`Firms`,`Average Gross Receipts`, `Average Gross Receipts in NCSMNM addition`), ~ format(.x, big.mark = ","))) 
