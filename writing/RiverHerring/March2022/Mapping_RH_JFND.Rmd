---
title: "Maps for River Herring: Jan, Feb, Nov, Dec "
author: Min-Yang Lee
...
<!-- 
This is the markdown document used to make the raster maps for the MAMFC River Herring Request.  It uses the offshoreWind package.

# The yearly binning has been done: 
2008-2011
2012-2015 
2016-2019 

Confined to only Jan, Feb, Nov, and December. The logical subset might not work if MONTH is string (instead of numeric)
-->
...
wind_area_hard_name = "River Herring and Shad"
...
root.geotiff.path<-file.path(project_root_path,"geotiffs","RiverHerring","March2022")
root.image.path = file.path(project_root_path,"images","RiverHerring","March2022")
root.table.path = file.path(project_root_path,"tables","RiverHerring","March2022")
...
overlap_data_path = file.path( project_root_path, "raw_data", "RiverHerring", "July2021") # the file path of the folder where the overlap files are. This comes from the output_folder of getCallAreaEffort
...
```{r getcallArea, eval = FALSE}  
#Plopping this in here, so I know how to find it if I have to update.  It looks like it took 5-10 hours per area, per year on MARS with 8 cores.  So that's a long time.
    get_overlays(
...
```{r load_libraries_and_sign_in_to_Google, eval=TRUE}  
...
```{r LOAD_EXISTING_GIS_DATA, eval=TRUE}
...
# River Herring Shad  areas
basemap_RHS = readOGR(dsn=file.path(project_root_path,"input_data", "RiverHerring", "July2021"), layer="RHS_combined", verbose=F)
basemap_RHS = spTransform(basemap_RHS, CRS=PROJ.USE)

#Split apart RHS areas
RHS1 = basemap_RHS[basemap_RHS$Id==1,]
RHS2 = basemap_RHS[basemap_RHS$Id==2,]
RHS3 = basemap_RHS[basemap_RHS$Id==3,]
RHS4 = basemap_RHS[basemap_RHS$Id==4,]
...
shapefile_path = file.path(project_root_path,"input_data", "RiverHerring", "July2021") 
...
output_folder_name = file.path(project_root_path,"raw_data/RiverHerring/March2022")
...
```{r load_VTR_REC_or_DMIS_flat_files_and_clean, eval=TRUE, include=FALSE}  
...
```{r add_mesh_size, eval=TRUE, include=FALSE}

#############################################################################

for(i in START.YEAR:END.YEAR) {
  #print(i)
  CONN<-dbConnect(drv, id, password=solepw, dbname=sole.connect.string)
 ...
 YEAR.RESULT = dbGetQuery(CONN, CURRENT.QUERY)  ### seems to be a problem with having a "paste" on both sides...
 dbDisconnect(CONN)
...
QUERY_RESULT$YEAR_L <- NULL # remove year variable
# Some ambiguity on ", " vs "," in FIN$GEARNM. 
FIN$GEARNM<-gsub(", ", ",",FIN$GEARNM)
FIN$GEARCAT <- ""
FIN$GEARCAT[FIN$GEARNM=="POT,LOBSTER"] <- "Lobster Pot"
FIN$GEARCAT[FIN$GEARNM %in% c("OTTER TRAWL,BOTTOM,FISH","OTTER TRAWL,BEAM","OTTER TRAWL,BOTTOM,OTHER", "OTTER TRAWL,BOTTOM,TWIN", "SEINE,DANISH","SEINE,SCOTTISH","PAIR TRAWL,BOTTOM") ] <- "Bottom Trawl"
...
FIN$GEARCAT[FIN$GEARNM=="PURSE SEINE"] <- "Purse Seine"
...
test_merge<-FIN[FIN$GEARCAT %in%c("Midwater Trawl" ,"Bottom Trawl","Sink Gillnet ","Other Gillnet ","Purse Seine"),]


#Check that merge is good for these categories.                 
missings<-sum(is.na(test_merge$MESH))
total<-nrow(test_merge)

if (missings/total>0.01){
stop("Matching Mesh resulted in too many missing values")
}         
...
if (noaa_data_source == "dmis") {
  ...
  # dim(FIN)
  FIN <- subset(FIN, select=c('PORTLANDED', 'PORTLND1', 'YEAR', 'MONTH', 'DATESAIL', 'GEARCODE', 'GEARCAT', 'NESPP3', 'SPPNM', 'QTYKEPT', 'LIVE', 'REVENUE', 'FMP', 'IDNUM', 'SERIAL_NUM', "DOCID", 'TRIPID', 'PERMIT', "STATE1", "SOURCE", "DEALNUM", "DAS",'LAT','LON', 'MESH')) # *added STATE1 variable  added MESH variable.
}
...
```{r adjust_nominal_values_to_real_values, eval =TRUE} 
...
#End of the adjust_nominal_values_to_real_values chunk.
```

```{r create_map_of_lease_area, eval = TRUE} 
...
if (any(proj_area_name %in% formatted_lease_area_table$proj_area_file_name) == FALSE & shapefile_path != "") {
  ...
  lease_area_map <- ggplot(data = east_coast) + 
    geom_sf(data = lease_area,  fill = c("green","red","blue","khaki2") )+
    geom_polygon(aes(x = long, y = lat, group = group), fill = "#D1D1E0", color = "black") +
    coord_sf(xlim = xlim, ylim = ylim) +
    ggtitle(wind_area_name) +
    theme(axis.title.y=element_blank(),
...
#End of the create_map_of_lease_area chunk.
...
logical_subset<-quote(which(FINALstack$GEARCAT %in% c("Midwater Trawl") &FINALstack$REGIME %in%c("1","2","3") & FINALstack[[FIELD]]>=1)) 
my_margin_name<-quote(paste(FINALstack$MONTH,readable_name, "REGIME",FINALstack$REGIME, sep="_"))
...
FIN$REGIME = dplyr::case_when(
          (FIN$Year >= 2008) & (FIN$Year <= 2011) ~ 1,
          (FIN$Year >= 2012) & (FIN$Year <= 2015) ~ 2,
         (FIN$Year >= 2016) & (FIN$Year <= 2019) ~ 3)
#HERE HERE HERE
...
#End of the prep_geotiffs1 chunk.
...
#End of the makegeotiffs1 chunk.
...
parsed_list1$REGIME = as.numeric(sapply(parsed_list1$NAME, USE.NAMES=F, function(zz) {
  temp = do.call(rbind,strsplit(as.character(zz), split = "_"))
  return(temp[4]) }))
...
# Keep months 1, 2, 11, 12

for (myregime in 1:3) {
   yearfileswanted <- parsed_list1[which(parsed_list1$REGIME==myregime & parsed_list1$MONTH %in% c(1,2,11,12)),]
if (nrow(yearfileswanted)==0){
#If there are no matching files, do nothing
  }
else{   
# If there is at least 1 matching file, build the name
outfilename<-paste0(outfilestub,"Regime_",myregime,".tif")
...
nobs<-length(unique(yearfileswanted$YEAR))
#  or don't divide by the number of years 
nobs<-1
holding_summer<-holding_summer/nobs
writeRaster(holding_summer, filename=file.path(out.geotiff.path,outfilename), overwrite=TRUE)
}
}
#End of the secondary_aggregation chunk.
...
parsed_groups$REGIME<-as.numeric(parsed_groups$REGIME)  
    ...
    parsed_groups$METRIC =  sapply(parsed_groups$NAME, USE.NAMES=F, function(zz) {
     ...
     parsed_groups$Type_Metric_Extra <-paste0(parsed_groups$Type_Metric_Extra,"_regime_",parsed_groups$REGIME)
            parsed_groups$Type_Metric_Extra <-"MWT"

            # Set this column to have the same value for all rows to graph things all at once.
...
xxyy=data.frame() # Start with creating a new list for dimension info
  ...
  #Set the bounding box by hand
  byhand.y<- c(100000,640818)
  byhand.x<- c(1778544, 2348544)
...
logical_subset<-quote(which(FINALstack$GEARCODE %in% c("MWT") &FINALstack$REGIME %in%c("1","2","3")& FINALstack$MONTH %in% c(1,2,11,12) & FINALstack[[FIELD]]>=1)) 
...
# We also keep only the entries where FINAL_W$MONTH matches
FINAL_W<-FINAL_W[which(FINAL_W$REGIME %in% working_group$REGIME),]
...
# Min-Yang's code changes for reclassification to 1-6 from value bins
  ...
  working_regime<-as.numeric(temp_R_list$REGIME[t])
...
# Now set up to plot this GROUP of rasters, with the above-determined binning scheme
  ...
  my.month<-temp_R_list$REGIME[t]
  my.month[which(my.month==0)]<-"N/A"
  my.month<-paste0("Period=",my.month)
...
RHS4 = basemap_RHS[basemap_RHS$Id==4,]
      ...
      print(jcons+ latticeExtra::layer(sp.lines(CAI_grnd)) + latticeExtra::layer(sp.lines(CAII_grnd))
            + latticeExtra::layer(sp.lines(NLS_grnd))+ latticeExtra::layer(sp.lines(RHS1)) + latticeExtra::layer(sp.lines(RHS2))
            +latticeExtra::layer(sp.lines(RHS3))+latticeExtra::layer(sp.lines(RHS4)) + 
              latticeExtra::layer(sp.polygons(basemap_eastcoast, fill=mylandfill))
...
#End of the check_confidential_secondary_aggregation chunk.

```	
<!--
This works properly. You will need to change some bits in the next set of 
End section to construct and map aggregated rasters for Small Mesh Bottom Trawl -->

<!-- Begin section to construct and map aggregated rasters for Small Mesh Bottom Trawl -->

```{r prep_geotiffs2, include=FALSE,eval=eval_switch2}   
# Read in the file attributes of the individual rasters.
fl<-readRDS(payloadRDS)
...
#Give the files and folders a pretty name.
readable_name ="smOTF"
logical_subset<-quote(which(FINALstack$GEARCAT %in% c("Bottom Trawl") & FINALstack$MESH <=3.5 & FINALstack$REGIME %in%c("1","2","3") & FINALstack[[FIELD]]>=1)) 
my_margin_name<-quote(paste(FINALstack$MONTH,readable_name, "REGIME",FINALstack$REGIME, sep="_"))
...
# This construction first rows that match the logical. So, we only get rows of smallmesh bottom trawl.
...
#End of the prep_geotiffs2 chunk.

```	

```{r makegeotiffs2, include=FALSE, eval=eval_switch2}  
source(file.path(project_root_path,"R_code/Step_4_generic_raster_aggregator.R"))
#End of the makegeotiffs2 chunk.

```	

```{r secondary_aggregation_part2, include=FALSE, eval=eval_switch2} 


#################################################
# STEP 3 -- Set your mapping options
#### WHAT ARE YOU TRYING TO Aggregat fike?
...
#End of the secondary_aggregation_part2 chunk.

```	


```{r check_confidential_secondary_aggregation2, include=FALSE, eval=eval_switch2} 
#file path with geotiffs that you need to check for confidentiality
in.geotiff.path = file.path(root.geotiff.path,paste(readable_name,todaysdate,sep="_"),"aggregated")
...
parsed_groups$REGIME<-as.numeric(parsed_groups$REGIME)  
    ...
    parsed_groups$METRIC =  sapply(parsed_groups$NAME, USE.NAMES=F, function(zz) {
     ...
     parsed_groups$Type_Metric_Extra =  sapply(parsed_groups$NAME, USE.NAMES=F, function(zz) {
       ...
       return(file.path(paste0(temp[1],"_",temp[2],"_",temp[3]))) }) # Adding "-1" moves you back one "/"  - as in, back one "/" separator to get the group/file folder name
     parsed_groups$Type_Metric_Extra <-paste0(parsed_groups$Type_Metric_Extra,"_regime_",parsed_groups$REGIME)
            parsed_groups$Type_Metric_Extra <-"smOTF"
...
TME<-working_group$Type_Metric_Extra[1]
logical_subset<-quote(which(FINALstack$GEARCODE %in% c("smOTF") &FINALstack$REGIME %in%c("1","2","3")& FINALstack$MONTH %in% c(1,2,11,12)& FINALstack[[FIELD]]>=1)) 
...
# We also keep only the entries where FINAL_W$MONTH matches
FINAL_W<-FINAL_W[which(FINAL_W$REGIME %in% working_group$REGIME & FINAL_W$MONTH %in% c(1,2,11,12)),]
...
##### 
      ...
      print(rclassed+ latticeExtra::layer(sp.lines(CAI_grnd)) + latticeExtra::layer(sp.lines(CAII_grnd))
            + latticeExtra::layer(sp.lines(NLS_grnd))+ latticeExtra::layer(sp.lines(RHS1)) + latticeExtra::layer(sp.lines(RHS2))
            +latticeExtra::layer(sp.lines(RHS3))+latticeExtra::layer(sp.lines(RHS4)) + 
              latticeExtra::layer(sp.polygons(basemap_eastcoast, fill=mylandfill))            #+ latticeExtra::layer(grid.text(my.regime,draw=TRUE,x=unit(0.80, "npc"),y=unit(0.10, "npc"), just="left",gp=gpar(fontsize=12, cex=1.5)))
            #+ latticeExtra::layer(grid.text(my.gear,draw=TRUE,x=unit(0.82, "npc"),y=unit(0.075, "npc"), just="left",gp=gpar(cex=1.5)))
            + latticeExtra::layer(grid.text(my.month,draw=TRUE,x=unit(0.80, "npc"),y=unit(0.05, "npc"), just="left",gp=gpar(fontsize=12, cex=1.5)))
...
#End of the check_confidential_secondary_aggregation2 chunk.

```	
<!-- Begin section to construct and map aggregated rasters for revenue on trips that retain herring -->
...
```{r RHS_MWT, fig.show = "hold", out.width = "48%", fig.cap="\\label{RHS_mwt} Total Revenue by Midwater Trawl. January, February, November, and December only. Top Left: 2008-2011. Top Right: 2012-2015.  Bottom: 2016-2019.",  fig.align = "center", echo=FALSE}
cleaned.image.path1<-file.path(root.image.path,paste("MWT",todaysdate,sep="_"),"PII_aggregated")

regimes<-c("1","2","3")
knitr::include_graphics(file.path(cleaned.image.path1,paste0("reclass_MWT_Regime_", regimes,".png")))
```
\newpage

## Small Mesh Bottom Trawl River herring


```{r RHS_smOTF, fig.show = "hold", out.width = "48%", fig.cap="\\label{herring_only} Total Revenue by Small Mesh Bottom Trawl.  January, February, November, and December only.  Top Left: 2008-2011. Top Right: 2012-2015.  Bottom: 2016-2019.",  fig.align = "center", echo=FALSE}
regimes<-c("1","2","3")
cleaned.image.path2<-file.path(root.image.path,paste("smOTF",todaysdate,sep="_"),"PII_aggregated")

knitr::include_graphics(file.path(cleaned.image.path2,paste0("reclass_smOTF_Regime_", regimes,".png")))
```
\newpage
