---
title: "Tables and Reports for Small Mesh Bottom Trawl -  River Herring Areas. Jan, Feb, Nov, Dec only."
author: Min-Yang Lee
...
---
<!-- 
This is the markdown document used to make the generic report for the MAMFC River Herring Request.  It uses the offshoreWind package.  You run this once to construct the standalone tables smOTF_table_update and MWT_table_update
Then you turn the 

select_gear_custom=FALSE
and subset_months=TRUE

to run the report for just month<=2 or month>=11


#Need to keep only  "TRAWL-MIDWATER" and a subset of bottom trawl 
...
TO do: 
Keep just midwater trawl and small mesh BT.
...
```{r load_VTR_REC_or_DMIS_flat_files_and_clean, eval = T, include=FALSE}  
...
```{r add_mesh_size, eval=T, include=FALSE}
...
```{r subset_MWT, eval=F}
REVENUEFILE<-REVENUEFILE[which(REVENUEFILE$GEARCAT %in% c("Midwater Trawl")),] 
REVENUEFILE_backup<-REVENUEFILE

FIN<-FIN[which(FIN$GEARCAT %in% c("Midwater Trawl")),] 
FIN_backup <- FIN
...
```{r subset_smOTF, eval=T}
REVENUEFILE<-REVENUEFILE[which(REVENUEFILE$GEARCAT %in% c("Bottom Trawl") & REVENUEFILE$MESH <=3.5),]
REVENUEFILE_backup<-REVENUEFILE
FIN<-FIN[which(FIN$GEARCAT %in% c("Bottom Trawl") & FIN$MESH <=3.5),]
FIN_backup <- FIN

```


```{r subset_month, eval=subset_month_switch}
REVENUEFILE<-REVENUEFILE[which(REVENUEFILE$MONTH %in% c(1,2,11,12)),]
FIN<-FIN[which(FIN$MONTH %in% c(1,2,11,12)),]
...
Descriptions of Selected Fishery Landings and Estimates of Vessel Revenue from Selected Areas

Small Mesh Bottom Trawl only (<=3.5 inch mesh).Jan, Feb, Nov, December only.

**Please keep in mind that we confined our analysis to only trips that fished with small mesh bottom trawl gear in January, February, November, and December only. This means that, while we have figures that describe impacts in percentage terms (say, the percentage of revenues by a permit that are in a particular area), the denominator in those sections is "small mesh bottom trawl revenue during those months periods" and not "all fishing revenue"**
...
* All summaries presented here are built from percentages of a trip that overlapped spatially with the areas. These percentages were applied to landings and values for that trip and summed. This differs from simply using the self-reported VTR/clam logbook locations as those place all value from that trip at a single point. Use of the VTR raster model is more representative as smoothing reported locations reduces the effect of location inaccuracy.
...
################################################################
  ...
  select_gear_codes$REGIME = dplyr::case_when(
          (YEAR >= 2008) & (YEAR <= 2011) ~ 1,
          (YEAR >= 2012) & (YEAR <= 2015) ~ 2,
...
write.csv(final_table, file=file.path(root.table.path,paste0("smOTF_table_update",todaysdate,".csv")), row.names=FALSE)
