...
Jason: My initial thought was looking at small mesh bottom trawl and mid-water trawl revenue maps, all species, (three time periods * 2 gears = 6 maps) that had the same 4 areas (green dashed) noted as my memo similarly outlined (I just drew them in over TMS lines).

    Small mesh OTF might be somewhat tricky. 
I'm getting some "timeout" problems because the parts that check the rasters take too long to run.  The solution is to
1. run the code chunks by hand.
2. Set all the time consuming chunks to eval=FALSE
3. Knit
-->
...
root.geotiff.path<-file.path(project_root_path,"geotiffs","RiverHerring","July2021")
root.image.path = file.path(project_root_path,"images","RiverHerring","July2021")
root.table.path = file.path(project_root_path,"tables","RiverHerring","July2021")
...
output_folder_name = file.path(project_root_path,"raw_data/RiverHerring/July2021")
...
for (myregime in 1:3) {
   yearfileswanted <- parsed_list1[which(parsed_list1$REGIME==myregime),]
...
logical_subset<-quote(which(FINALstack$GEARCODE %in% c("MWT") &FINALstack$REGIME %in%c("1","2","3") & FINALstack[[FIELD]]>=1)) 
...
TME<-working_group$Type_Metric_Extra[1]
logical_subset<-quote(which(FINALstack$GEARCODE %in% c("smOTF") &FINALstack$REGIME %in%c("1","2","3") & FINALstack[[FIELD]]>=1)) 
...
```{r RHS_MWT, fig.show = "hold", out.width = "48%", fig.cap="\\label{RHS_mwt} Total Revenue by Midwater Trawl. Top Left: 2008-2011. Top Right: 2012-2015.  Bottom: 2016-2019.",  fig.align = "center", echo=FALSE}
...
```{r RHS_smOTF, fig.show = "hold", out.width = "48%", fig.cap="\\label{herring_only} Total Revenue by Small Mesh Bottom Trawl.Top Left: 2008-2011. Top Right: 2012-2015.  Bottom: 2016-2019.",  fig.align = "center", echo=FALSE}
regimes<-c("1","2","3")
