...
raster_manifest_location<-file.path(network_location,"home2/mlee/READ-SSB-Lee-Raster_requests/raw_data")
...
ML.OUTPUT.GEOTIFF.PATH<-file.path(network_location,"home2/mlee/READ-SSB-Lee-Raster_requests/geotiffs")
...
source(file.path(network_location,"/home2/mlee/READ-SSB-Lee-Raster_requests/R_code/raster_manifest.R")
```
...
oracle_overlap_table = "APSD.ALL_WEA_2008_2019@garfo_nefsc.world" #Oracle table to pull overlay files from
 overlap_data_path = file.path(network_location,"home5","dcorvi","geret_data_request","HerringFW7_output_CW")
...
shapefile_path = file.path(network_location,"home2","mlee","","READ-SSB-Lee-Raster_requests/input_data/Herring/Framework7")
...
output_folder_name = file.path(network_location,"home2/mlee/READ-SSB-Lee-Raster_requests/raw_data/Herring/Framework7")
# used for naming the folder where files will be saved

# Plot colors
assignedColors <- FALSE # assign unique colors for each margin in plots
...
message("Running lease area(s): ", wind_area_name, ", Project Area(s): ", proj_area_name," ..")
...
FIN <- subset(FIN, select=c('PORTLANDED','YEAR','VESID','NESPP3','MONTH','GEARCODE','GEARCAT','SERIAL_NUM','IDNUM','LIVE',
                            'QTYKEPT','QTYDISC','DAS','SPPNM','REVENUE','FMP','LEN','FY','TRIPID','PERMIT',"STATE1"))
} 

if (noaa_data_source == "rec") {
FIN <- subset(FIN, select=c('TRIPID', 'IDNUM', 'YEAR', 'MONTH', 'DAS', 'SPPNM', 'FMP', 'NANGLERS', 'LIVE', 'QTYKEPT','QTYDISC', 'PORTLANDED',
                            'NESPP3', 'GEARCODE', 'GEARCAT', 'PERMIT', 'VESID', 'SERIAL_NUM', "STATE1", "DATESAIL"))
} 
...
if (noaa_data_source == "dmis") {
  ...
  FIN <- subset(FIN, select=c('PORTLANDED', 'PORTLND1', 'YEAR', 'MONTH', 'DATESAIL', 'GEARCODE', 'GEARCAT', 'NESPP3', 'SPPNM', 'QTYKEPT', 'LIVE',
                              'REVENUE', 'FMP', 'IDNUM', 'SERIAL_NUM', "DOCID", 'TRIPID', 'PERMIT', "STATE1", "SOURCE", "DEALNUM", "DAS")) # *added STATE1 variable
}
...
all_areas <- spTransform(offshoreWind::all_areas, CRS=CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
...
if (any(proj_area_name %in% formatted_lease_area_table$proj_area_file_name) == FALSE & shapefile_path != "") {
  ...
  for (layer in layer_name) {
    if (count == 0) {
      ...
      all_areas <- union(extra_layer, all_areas) # Combine all lease areas plus maine area into one shapefile
    }
  }
  ...
  lease_area_map <- ggplot(data = east_coast) + 
    geom_polygon(aes(x = long, y = lat, group = group), fill = "#D1D1E0", color = "black") +
    geom_sf(data = lease_area,  color = "green") +
    coord_sf(xlim = xlim, ylim = ylim) +
    ggtitle(wind_area_name) +
    theme(axis.title.y=element_blank(),
          axis.title.x=element_blank(),
...
readable_name ="herring_only"
my_margin_name<-quote(paste(FINAL$MONTH,readable_name, sep="_"))

#You're going to have need a better name here. 
export.geotiff.path = file.path(ML.OUTPUT.GEOTIFF.PATH,paste0("FW7_2019-12-10"))
...
source(file.path(network_location,"home2/mlee/READ-SSB-Lee-Raster_requests/R_code/Step_4_generic_raster_aggregator.R"))
...
[Benjamin S, Lee MY, DePiper G. 2018. Visualizing fishing data as rasters. NEFSC Ref Doc 18-12; 24 p.](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12959)
...
# get top 5 fmps by revenue, by area (without confidentiallity check) for dynamic text
top5byArea <- REVENUEFILE %>% group_by(FMP, Area) %>%
  filter(Area != "Other") %>% 
  dplyr::summarize(
    REV = sum(InsideREV, na.rm = T)
    ) %>%
  ungroup %>%
  group_by(Area) %>%
  top_n(., n=5, wt=REV) %>%
...
top_five_fmp_names <- subset(REVENUEFILE, !is.na(FMP)) %>%
  filter(Area != "Other") %>% 
...
# check for whether FMPs are all the same between areas then set text to be dynamic and inform users what areas where not ubiquitous
# check_for_unique_FMPS <- ifelse(any(as.integer(unique(top5byArea$count)) %in% 1), paste0("Some ", area_adjective, "areas did not share the same top five most impacted FMPs. ", "The ", area_adjective, "area(s) with differing FMPs were ", gsub("_"," ", pull(top5byArea[top5byArea$count==1,"Area"])), " and the FMP(s) include ", pull(top5byArea[top5byArea$count==1,"FMP"]),"."), paste0("All ", area_adjective, "areas had the same top five most impacted FMPs."))

# pull(top5byArea[top5byArea$count==1,"FMP"]) # get vector of FMPs not shared by all areas
# pull(top5byArea[top5byArea$count==1,"Area"]) # get vector of Area not shared by all areas

# paste0("Area(s) with differing FMPs were ",gsub("_"," ", pull(top5byArea[top5byArea$count==1,"Area"])), " with FMPs that include ", pull(top5byArea[top5byArea$count==1,"FMP"]),".")

# Might be an issue of top 5 FMP has 0 revenue

piifmp_allyrs <- subset(REVENUEFILE, !is.na(FMP)) %>%
  filter(Area != "Other") %>% 
  # dplyr::select(FMP, Year, InsideREV, InsideLANDED, PERMIT ) %>%
  ...
  dplyr::rename("FMP"= FMP_CONF, "All Revenue" = InsideREV_total, "All Landings (Pounds)" = InsideLANDED_total)  

allyrfmp_table <- head(piifmp_allyrs[order(-piifmp_allyrs$'All Revenue'),],5) #get top FMPs of all areas by revenue

all_topfmp_names <- unique(allyrfmp_table$FMP)

all_topfmp_names <- top_five_fmp_names # pii after grouping fix for all areas

# allyrfmp_table$FMP <- gsub("(?<=\\b)([a-z])", "\\U\\1", tolower(allyrfmp_table$FMP), perl=TRUE)
# allyrfmp_table$FMP[allyrfmp_table$FMP=="None"] <- "No Federal FMP"

topfmp_names_all <- unique(allyrfmp_table$FMP)
topfmp_names_all <- top_five_fmp_names # pii after grouping fix for all areas

if (any(topfmp_names_all %in% "No Federal FMP")) {
  # allyrfmp_table$FMP[allyrfmp_table$FMP=="None"] <- "No Federal FMP"
  topfmp_names <- paste0(topfmp_names_all[!topfmp_names_all %in% "No Federal FMP"], collapse = ", ")
  topfmp_names <- paste0(topfmp_names, " and No Federal FMP")
} else {
  topfmp_names <- paste0(topfmp_names_all %notin%  tail(topfmp_names_all, n=1), collapse = ", ")
  topfmp_names <-paste0(topfmp_names, " and ", tail(topfmp_names_all, n=1))
}

# length(unique(REVENUEFILE[which(REVENUEFILE$FMP == "None" & REVENUEFILE$InsideREV > 0),]$SPPNM))

piifmpsppnm_allyrs <- subset(REVENUEFILE, !is.na(FMP)) %>% 
  filter(Area != "Other") %>% 
...
# ifelse(any(topfmp_names_all %in% "No Federal FMP"), paste0(" and “No Federal FMP.” The category “No Federal FMP” contains a variety of species that are not federally regulated, such as: lobster, Jonah crab, smooth and chain dogfish, whelk, and menhaden, (",ifelse(numsp==0,"there were no species", paste0("there are close to ", numsp," species"))," without federal FMPs caught in the ", wind_area_name, "  area)."),".")


# ifelse(any(topfmp_names_all %in% "No Federal FMP"),paste0(" and “No Federal FMP.” The category “No Federal FMP” contains a variety of species that are not federally regulated, such as: lobster, Jonah crab, smooth and chain dogfish, whelk, and menhaden, (there are close to ", numsp," species without federal FMPs caught in ----)."),".")

# what if areas dont have same top FMP
...
<!-- add check_for_unique_FMPS object here if nessessary -->
<!-- ~~All areas had the same top five most impacted FMPs~~ -->
 The top 5 FMPs by revenue `r if(number_of_areas > 1) {paste0("between all areas")} else {paste0("in ", wind_area_name)}` were `r topfmp_names``r ifelse(any(topfmp_names_all %in% "No Federal FMP"), paste0(".” The category “No Federal FMP” contains a variety of species that are not federally regulated, such as: smooth and chain dogfish, whelk, and menhaden, (",ifelse(numsp==0,"there were no species", paste0("there are close to ", numsp," species"))," without federal FMPs caught in the ", wind_area_name, " area)"),"")`. `r revenue_deflation_note` `r rounded_message` Specific figures on these FMPs within `r if(number_of_areas > 1) {paste0("each")} else {paste0("the")}` area follow.


```{r top_fmp_plots, results='asis', eval = T} 
##########################################################################
# testing
...
allYearsSum <- subset(REVENUEFILE, !is.na(FMP)) %>% # FMP NAs are already removed in the DMIS cleaning code chunk
  filter(Area != "Other") %>% 
...
piifmp <- subset(REVENUEFILE, !is.na(FMP)) %>%
  filter(Area != "Other") %>% 
  # dplyr::select(FMP, Year, InsideREV, InsideLANDED, PERMIT, Area ) %>%
...
# write.csv(piifmp_allyrs, file="piifmp_allyrs.csv", row.names=FALSE)
...
topFMP_all_areas <- left_join(allYearsSum, fmp_merge, by='FMP') %>% 
  filter(FMP %in% all_topfmp_names) # all_topfmp_names is from the summary creater section 

topFMPNames <- all_topfmp_names # rename to make it sound better
...
```{r top_fmp_summary_2, results='asis', eval = summary_tables} 
...
if (number_of_areas == 1) {
  all_rev <- allyrrev_table %>% filter(FMP =="**Total**") %>% select(-FMP)
  all_land <- allyrland_table %>% filter(FMP =="**Total**") %>% select(-FMP)
  rev_land_total <- cbind(all_rev, all_land)
  row.names(rev_land_total) <- "**Total**"
  print(kable(rev_land_total, caption = paste0(numyear,' Year Landings and Revenue, ', 'Most Impacted FMPs '), align=c('r', 'r')))
} else {
  counter = 1
  for (ar in allZones) {
    if (counter == 1) {
    ar_u <- gsub("(?<=\\b)([a-z])", "\\U\\1", gsub("\\s","_", gsub("/","-", ar)), perl=TRUE)
    allyr_table <- topFMP_all_areas %>%  
      filter(Area == ar_u) %>% 
      distinct(FMP, Area, allyrtotal_rev, allyrtotal_land) %>%
      arrange(desc(allyrtotal_land)) %>%
      dplyr::select(FMP, Area, allyrtotal_rev, allyrtotal_land) %>% 
      dplyr::rename("All Year Revenue" = allyrtotal_rev, "All Year Landings (Pounds)" = allyrtotal_land) %>% 
      mutate(FMP = as.character(FMP)) %>% 
      ungroup()
    allyr_table <- rbind(allyr_table, c("**Total**", ar, colSums(allyr_table[,3]), colSums(allyr_table[,4])))
    allyr_table <- allyr_table %>% filter(FMP =="**Total**")
    counter = 2
  } else {
    ar_u <- gsub("(?<=\\b)([a-z])", "\\U\\1", gsub("\\s","_", gsub("/","-", ar)), perl=TRUE)
      
    allyr_table_2 <- topFMP_all_areas %>%  
      filter(Area == ar_u) %>% 
      distinct(FMP, Area, allyrtotal_rev, allyrtotal_land) %>%
      arrange(desc(allyrtotal_land)) %>%
      dplyr::select(FMP, Area, allyrtotal_rev, allyrtotal_land) %>% 
    ...
    allyr_table_2 <- allyr_table_2 %>% filter(FMP =="**Total**")
  ...
  allyr_table[nrow(allyr_table),4] <- paste0("**", allyr_table[nrow(allyr_table),4],"**")
  ...
  print(kable(allyr_table, caption = paste0('Most Impacted FMPs ', numyear,' Year Landings and Revenue '), align=c('l', 'r', 'r')))  
...
```{r top_fmp_summary, results='asis', eval = F} 
#####################################################################
################### Summary Section #################################
#####################################################################
cat("*Summary*")

# Total Revenue and Landings by Top 5 Most Impacted FMPs 
...
# create list aggrigated by year and FMP of FMPs with PII sensitive listings removed 
piifmp_allyrs <- subset(REVENUEFILE, !is.na(FMP)) %>%
  # dplyr::select(FMP, Year, InsideREV, InsideLANDED, PERMIT ) %>%
  dplyr::select(FMP, Year, InsideREV, InsideLANDED, PERMIT, DEALNUM) %>% # for dealers
  group_by(FMP) %>%
  ...
  ungroup() %>%
  # dplyr::select(-starts_with("FMP")) %>%
  dplyr::select(InsideREV, InsideLANDED, npermits, ndealers, FMP_CONF) %>%
  group_by(FMP_CONF) %>%
  dplyr::summarize(
    InsideREV_total = sum(InsideREV, na.rm = T),
    InsideLANDED_total = sum(InsideLANDED, na.rm = T),
  ...
  arrange(desc( InsideREV_total, .by_group = TRUE)) %>%
  dplyr::rename("FMP"= FMP_CONF, "All Year Revenue" = InsideREV_total, "All Year Landings (Pounds)" = InsideLANDED_total)  
  # dplyr::rename("FMP"= FMP_CONF)

# TODO: create excel table
# remove permit number column
# piifmp_allyrs <- piifmp_allyrs[ , !(names(piifmp_allyrs) %in% "npermits" )]
# piifmp_allyrs <- piifmp_allyrs[ , !(names(piifmp_allyrs) %in% c("npermits","ndealers") )]
# remove extra columns
piifmp_allyrs <- piifmp_allyrs %>% dplyr::select("FMP", "All Year Revenue", "All Year Landings (Pounds)")

allyrfmp_table <- head(piifmp_allyrs[order(-piifmp_allyrs$'All Year Revenue'),],5)

topfmp_names <- unique(allyrfmp_table$FMP)
...
allyrfmp_table <- rbind(allyrfmp_table, c("**Total**", colSums(allyrfmp_table[,2]), colSums(allyrfmp_table[,3])))
allyrfmp_table$`All Year Revenue` <- comma_format( prefix = "$", trim = FALSE, accuracy = 1000)(as.numeric(allyrfmp_table$`All Year Revenue`))
allyrfmp_table$`All Year Landings (Pounds)` <- comma_format(trim = TRUE, accuracy = 1000)(as.numeric(allyrfmp_table$`All Year Landings (Pounds)`))
# Make bottom rows bold
allyrfmp_table[nrow(allyrfmp_table),2] <- paste0("**",allyrfmp_table[nrow(allyrfmp_table),2],"**")
allyrfmp_table[nrow(allyrfmp_table),3] <- paste0("**",allyrfmp_table[nrow(allyrfmp_table),3],"**")

# set dynamic varible names (cant use paste0 in rename in tidyverse)
names(allyrfmp_table)[names(allyrfmp_table) == "All Year Revenue"] <- paste0(numyear, " Year Revenue")
names(allyrfmp_table)[names(allyrfmp_table) == "All Year Landings (Pounds)"] <- paste0(numyear, " Year Landings (Pounds)")

if (number_of_areas > 1) {
cat("\n\n")
print(kable(allyrfmp_table, caption = paste0("Most Impacted FMPs ", numyear, " Year Landings and Revenue, All ", proper(area_adjective), "Areas "), align=c('l', 'r', 'r')))
} # end number of areas check

cat("\n\n")

########## Sum Rev and Landings by area #############
allyrarea_table <- subset(REVENUEFILE, !is.na(FMP)) %>%
  # dplyr::select(FMP, Area, InsideREV, InsideLANDED, PERMIT ) %>%
  dplyr::select(FMP, Area, InsideREV, InsideLANDED, PERMIT, DEALNUM) %>% # for dealers
  filter(Area != "Other") %>% 
  filter(FMP %in% topfmp_names) %>% 
  group_by(Area) %>%
  dplyr::summarize(
    InsideREV = sum(InsideREV, na.rm = T),
    InsideLANDED = sum(InsideLANDED, na.rm = T),
  ...
  ungroup() %>%
  # dplyr::select(-starts_with("FMP")) %>%
  dplyr::select(Area, InsideREV, InsideLANDED, npermits, ndealers, Area_CONF) %>%
  group_by(Area_CONF) %>%
  dplyr::summarize(
    InsideREV_total = sum(InsideREV, na.rm = T),
    InsideLANDED = sum(InsideLANDED, na.rm = T),
    npermits = sum(npermits, na.rm = T),
    ndealers = sum(ndealers, na.rm = T)) %>%
  dplyr::rename("All Year Revenue" = InsideREV_total, "All Year Landings (Pounds)" = InsideLANDED, "Area" = Area_CONF)

# remove underscores
allyrarea_table$`Area` <- gsub("(?<=\\b)([a-z])", "\\U\\1", tolower(gsub("_"," ", allyrarea_table$`Area`)), perl=TRUE) # make name pretty

#TODO: create excel table??
# names(allyrarea_table)
# remove permit column
allyrarea_table <- allyrarea_table %>% dplyr::select('Area', 'All Year Revenue', 'All Year Landings (Pounds)')

# Create totals column
allyrarea_table <- rbind(allyrarea_table, c('**Total**', colSums(allyrarea_table[,2]), colSums(allyrarea_table[,3])))
# Format columns
allyrarea_table$`All Year Revenue` <- comma_format( prefix = "$", trim = FALSE, accuracy = 1000)(as.numeric(allyrarea_table$`All Year Revenue`))
allyrarea_table$`All Year Landings (Pounds)` <- comma_format(trim = TRUE, accuracy = 1000)(as.numeric(allyrarea_table$`All Year Landings (Pounds)`))
# Remove underscores from area names
allyrarea_table$`Area` <- gsub("_"," ", allyrarea_table$`Area`)
# allyrarea_table
# Make bottom rows bold
allyrarea_table[nrow(allyrarea_table),2] <- paste0("**",allyrarea_table[nrow(allyrarea_table),2],"**")
allyrarea_table[nrow(allyrarea_table),3] <- paste0("**",allyrarea_table[nrow(allyrarea_table),3],"**")

# set dynamic varible names (cant use paste0 in rename in tidyverse)
names(allyrarea_table)[names(allyrarea_table) == "Area"] <- paste0(proper(area_adjective), "Area")
# names(allyrarea_table)[names(allyrarea_table) == "All Year Total"] <- paste0(numyear, " Year Total")
names(allyrarea_table)[names(allyrarea_table) == "All Year Revenue"] <- paste0(numyear, " Year Revenue")
names(allyrarea_table)[names(allyrarea_table) == "All Year Landings (Pounds)"] <- paste0(numyear, " Year Landings (Pounds)")

if (number_of_areas > 1) {
  print(kable(allyrarea_table, caption = paste0('Most Impacted FMPs ', numyear,' Year Landings and Revenue '), align=c('l', 'r', 'r')))  
} else {
  single_area_table <- as.data.frame(allyrarea_table[-1,-1])
  row.names(single_area_table) <- "**Total**"
  print(kable(single_area_table, caption = paste0('Most Impacted FMPs ', numyear,' Year Landings and Revenue '), align=c('r', 'r')))
...
```{r echo=FALSE} 
# get other fmp names
# unique(fmp_merge$FMP) %notin% all_topfmp_names

otherfmpsnames <- gsub("(?<=\\b)([a-z])", "\\U\\1", tolower(unique(fmp_merge$FMP) %notin% all_topfmp_names), perl=TRUE)
otherfmpsnames[otherfmpsnames=="None"] <- "No Federal FMP"

# otherfmpsnames[length(otherfmpsnames)]

# ifelse(any(otherfmpsnames %in% "No Federal FMP"), paste0(" and “No Federal FMP.” The category “No Federal FMP” contains a variety of species that are not federally regulated, such as: lobster, Jonah crab, smooth and chain dogfish, whelk, and menhaden, (there are close to ", numsp," species without federal FMPs caught in ----)."),".")
# ifelse(any(otherfmpsnames %in% "All Others"), paste0(" The category \"All Others\" refers FMPs with less than three permits impacted"),".")

```


```{r other_impacted_fmps_dynamic_text, results='asis', eval = T} 
...
```{r other_impacted_fmps, results='asis', eval = T} 
...
otherfmpnames <- unique(fmp_merge$FMP) %notin% topFMPNames
...
```{r other_fmp_summary_2, results='asis', eval = summary_tables} 
...
if (number_of_areas == 1) {
  ...
  rev_land_total <- cbind(all_rev, all_land)
  row.names(rev_land_total) <- "**Total**"
  print(kable(rev_land_total, caption = paste0(numyear,' Year Landings and Revenue, ', 'Other Impacted FMPs '), align=c('r', 'r')))
}
...
# names(myColors) <- UniqueMargins # name colors for color scale
# 
# uniqueTopSpeciesScale <- scale_fill_manual(name = NULL, values = myColors, guide = guide_legend(reverse = TRUE))
...
if (number_of_areas == 1) {
  all_rev <- allyrrev_table %>% filter(Species =="**Total**") %>% select(-Species)
  all_land <- allyrland_table %>% filter(Species =="**Total**") %>% select(-Species)
  rev_land_total <- cbind(all_rev, all_land)
  row.names(rev_land_total) <- "**Total**"
  print(kable(rev_land_total, caption = paste0(numyear,' Year Landings and Revenue, ', 'Top Species '), align=c('r', 'r')))
}
...
if (number_of_areas == 1) {
  all_rev <- allyrrev_table %>% filter(`Gear Type` =="**Total**") %>% select(-`Gear Type`)
  all_land <- allyrland_table %>% filter(`Gear Type` =="**Total**") %>% select(-`Gear Type`)
  rev_land_total <- cbind(all_rev, all_land)
  row.names(rev_land_total) <- "**Total**"
  print(kable(rev_land_total, caption = paste0(numyear,' Year Landings and Revenue, ', 'Select Gear Code '), align=c('r', 'r')))
}
...
# PII fixed for all areas
piigearids <- subset(REVENUEFILE, !is.na(FMP)) %>%
...
# PII fixed for all areas
piipermitAllyear <- subset(REVENUEFILE, !is.na(FMP)) %>%
  dplyr::select(Year, REVENUE, InsideREV, InsideLANDED, PERMIT, Area) %>%
  # dplyr::select(FMP, Year, InsideREV, InsideLANDED, PERMIT, DEALNUM) %>% # for dealers
  # dplyr::select(Year, PERMIT, Area) %>%
  filter(Area != "Other") %>% 
...
vessel_trip_all_areas_by_year <- subset(REVENUEFILE, !is.na(FMP)) %>% 
...
vessel_trip_all_areas_by_year_fmp <- subset(REVENUEFILE, !is.na(FMP)) %>%
  # filter(Area == zone) %>% 
  filter(Area != "Other") %>% 
...
vessel_trip_all_areas_by_year_sppnm <- subset(REVENUEFILE, !is.na(FMP)) %>%
  # filter(Area == zone) %>% 
  filter(Area != "Other") %>% 
...
vessel_trip_all_areas_by_year_port <- subset(REVENUEFILE, !is.na(FMP)) %>%
  # filter(Area == zone) %>% 
  filter(Area != "Other") %>% 
