...
```{r prep_geotiffs1, include=FALSE, eval=TRUE}  
...
```{r makegeotiffs1, include=FALSE, eval=TRUE}  
...
```{r make_geotiffs2, include=FALSE,eval=TRUE}   
# Read in the file attributes of the individual rasters.
...
```{r makegeotiffs2, include=FALSE, eval=TRUE}  
source(file.path(project_root_path,"R_code/Step_4_generic_raster_aggregator.R"))

```	

```{r secondary_aggregation, include=FALSE, eval=TRUE} 
...
readable_name ="herring-only"
geotiff.path = file.path(root.geotiff.path,paste(readable_name,"2021-03-16",sep="_"))
sp1<-"herring-only_QTYKEPT_"
z<-paste0("*",sp1,"*.tif")
mypat1<-glob2rx(z)

outfilestub<-sp1
out.geotiff.path = file.path(root.geotiff.path,paste(readable_name,"2021-03-16",sep="_"),"aggregated")
...
list1<- lapply(as.list(list.dirs(path=geotiff.path, recursive=T)), list.files, recursive=F, full.names=T, pattern=mypat1)
...
```{r secondary_aggregation_part2, include=FALSE, eval=TRUE} 
